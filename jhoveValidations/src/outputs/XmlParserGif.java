...
public class XmlParserGif {
	...
	public static void parseXmlFile(String xmlfile) {
		...
		try {
			...
			String xmlxslStyleSheet = "<?xml-stylesheet type=\"text/xsl\" href=\"GifJhoveCustomized.xsl\"?>";
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					xmlsummary.println("<GifFile>");
					String testutf8 = eElement.getElementsByTagName("filename").item(0).getTextContent();

					if (testutf8.contains("&")) {
						String sub = validatorUtilities.genericUtilities.normaliseToUtf8(testutf8);
